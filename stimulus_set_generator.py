...
class StimulusMaker:
    ...
    def draw_shape(self, shape_id, offset_direction=0):
        ...
        if shape_id == 0:
            return self.draw_vernier(offset_direction, -1)
        if shape_id == 1:
            ...
            if idx == 0:
                ...
                if centralize:
                    ...
                    if reduce_df:
                        # We want to make the degrees of freedom for position on
                        # the x axis fair:
                        # TODO: Changed max(n_shapes) to 5
                        image_size_adapted = self.image_size[1] - (5 - selected_repetitions) * self.patch_size
                        image_start = int((self.image_size[1] - image_size_adapted) / 2)
                        col = np.random.randint(image_start,
                                                image_start + image_size_adapted - self.patch_size * selected_repetitions)
                        row = np.random.randint(0, self.image_size[0] - self.patch_size)
                        while (row, col) in used_coords:
                            print("Coordinate collision!", (row,col))
                            col = np.random.randint(image_start,
                                                    image_start + image_size_adapted - self.patch_size * selected_repetitions)
                            row = np.random.randint(0, self.image_size[0] - self.patch_size)
                        used_coords.append((row, col))
                    else:
        ...
        for idx_batch in range(batch_size):
            ...
            if centralize:
                ...
                if reduce_df:
                    # We want to make the degrees of freedom for position on
                    # the x axis fair:
                    image_size_adapted_1 = self.image_size[1] - (5 - selected_repetitions_1) * self.patch_size
                    ...
                    col_shape_1 = col_shape_1_init
                    image_size_adapted_2 = self.image_size[1] - (5 - selected_repetitions_2) * self.patch_size
...
def segmented_make_verniers_stimulus_set(batch_size, directory_name, mode='verniers', split=0.5):
    segment_size = 50
    try:
    ...
    for noise_level in ['low', 'medium', 'high']:
        ...
        for i in range(0, int(np.floor(batch_size * split))):
            if i % segment_size == 0 and i != 0:
                ...
                df_test.to_csv(directory + '_metadata/' + 'test_' + noise_level + '_' + str(test_segment_index) + '.csv')  # Export CSV
                stimuli_dict_test = {
                                        'stimulus_id': [],
                                        'filename': [],
                                        'resolution': [],
            ...
            position = (test_image_data[9][i][0], test_image_data[10][i][0])
            ...
            # noise
            if noise_level == 'low':
                noise = np.abs(np.random.random() / 7)
            elif noise_level == 'medium':
                noise = np.abs(np.random.random() * 2 / 7) + 1 / 7
            else:
                noise = np.abs(np.random.random() * 4 / 7) + 3 / 7
            ...
            filename = "test_" + str(test_index) + "_" + str(int(offset)) + "_" + value + "_" + noise_level + "_.png"
            ...
            stimuli_dict_test['noise'].append(noise_level)
            stimuli_dict_test['position'].append(position)
        ...
        for i in range(int(np.floor(batch_size * split)), batch_size):
            ...
            if i % segment_size == 0 and i != int(np.floor(batch_size * split)):
                ...
                df_train.to_csv(directory + '_metadata/' + 'train_' + noise_level + '_' + str(train_segment_index) + '.csv')  # Export CSV
                stimuli_dict_train = {
                                            'stimulus_id': [],
                                            'filename': [],
                                            'resolution': [],
            ...
            filename = "train_" + str(train_index) + "_" + str(int(offset)) + "_" + value + "_" + noise_level + "_.png"
            im.save(directory_name + "/" + filename)  # Save to directory
            ...
            stimuli_dict_train['noise'].append(noise_level)
            stimuli_dict_train['position'].append(position)
...
segmented_make_verniers_stimulus_set(5000, 'uncrowding', mode='uncrowding', split=0.2)
